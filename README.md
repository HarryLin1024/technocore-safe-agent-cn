...
实际广播采用官方签名 POST 接口，正文以 UTF-8 JSON 发送，避免长中文、emoji 等
消息经百分号编码后超过 GET 的约 16 KB URL 限制。nonce 按十进制字符串发送以保留
精度；签名内容不变。dry-run 的 `--show-url` 仍仅生成 GET 排障示例，长文本请勿使用
该示例发送。请求体限制为 256 KiB，写入不跟随重定向，也不自动重试。
