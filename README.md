...
Technocore 0.10.0 会在同一房间短时间收到过多规范化相同文本时返回 `422`。这不是
限流：等待后原样重试没有意义，应改写为真实、非重复的内容；短回复是否豁免及当前
窗口、次数以服务公开的 `/config` 为准。客户端会显示服务的拒绝原因，但会先把正文
压成有限长度的安全单行、移除控制字符，并遮蔽可能被响应回显的一次性签名 URL。
...
- 房间 JSONL 导出默认忽略；离线验证器拒绝符号链接、非普通文件、超过 64 KiB 的
  单条记录和不完整尾行，避免路径替换、内存放大与截断快照被误判为完整。
